# Detection and Incident Report Portfolio
...
![Wazuh Brute Force Overview Alert](brute-force-overview.png)
...
![Wazuh Brute Force Document Details](brute-force-details.png)

### Root Cause
Test activity using smbclient from Kali against the Windows target share with invalid credentials to verify Wazuh log ingestion.

### Analyst Assessment and Response
...
## Report 2: EICAR Test File Detection and SIEM Visibility Remediation
* Report ID: INC-002
* Date/Time: July 20, 2026, 20:42 UTC
* Severity: Low (Controlled AV test file)

### Summary
Microsoft Defender Antivirus detected and quarantined an EICAR test file downloaded to Victim-Windows via PowerShell. An initial telemetry gap was identified and remediated during testing.

![Windows Defender Host Quarantine Alert](defender-host-quarantine.png)
...
### Telemetry Gap and Remediation
The default Wazuh agent configuration (ossec.conf) did not include the Windows Defender operational channel. 

Fix: Added the Microsoft Defender Operational event channel location to ossec.conf and restarted the Wazuh agent service. End-to-end detection from Windows Event ID 1117 to Wazuh Rule 62124 was confirmed upon re-testing.

![Wazuh Event 1117 Ingestion Confirmation](wazuh-defender-event.png)
...
### Summary
An Nmap scan was executed from Kali (192.168.100.10) against Victim-Windows (192.168.100.20), successfully identifying open ports (135/msrpc, 139/netbios-ssn, 445/microsoft-ds). However, no alerts were generated in Wazuh.

### Root Cause Analysis
1. Agent Query Exclusion: The default ossec.conf excludes Windows Filtering Platform events (Event IDs 5145 and 5156).
2. Audit Policy Limitations: Windows advanced network object auditing is disabled by default.
3. Host-Based Boundary: Raw TCP SYN handshakes do not generate host authentication or process execution events.
